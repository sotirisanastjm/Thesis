...
const Login = (props: { active: boolean; onPopUpChange: (newState: boolean) => void }) => {
    ...
    const loginWithWallet = async (walletAddress: string) => {
        try {
            const response = await fetch("https://localhost:7261/api/Auth/authenticate", {
                method: "POST",
                headers: {
    ...
    const handleLogin = async () => {
        try {
            const data: {token: string, User: User} = await loginWithWallet(walletAddress);
        ...
        <div>
            ...
            {popUp && (
                <div className={`fixed inset-0 backdrop-blur-sm z-10 h-full w-full flex justify-center items-center`}>
                    <div className={`transition-transform transform duration-1000 ease-in-out  ${popUp ? 'scale-100' : 'scale-50'} w-[80%] h-[50%] lg:w-[50%] bg-dark-700 rounded-2xl shadow-2xl shadow-dark-600 relative`}>
                        <button onClick={() => {setPopUp(false)}} className='absolute p-4 right-0 hover:cursor-pointer'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
