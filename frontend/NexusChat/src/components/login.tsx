...
import { signTransaction, executeCreateUser } from '../services/TransactionService';
import { LoginWithWallet, RegisterUser, ValidateAddress } from '../services/LoginService';
...
const Login = (props: { active: boolean; onPopUpChange: (newState: boolean) => void }) => {
    ...
    const validate_Address = async () => {
        try {
            if (formData.walletAddress) {
                const result = await ValidateAddress(formData.walletAddress);
        ...
        try {
            if (walletStatus.userExist) {
                ...
                if (formData?.walletAddress != "" && formData?.email != "" && formData?.username != "" && formData?.password != "") {
                    ...
                    if (data) {
                        const transactionUserData = await signTransaction(data?.txBytesUser);
                        const transactionUserDataResponse = await executeCreateUser(transactionUserData, formData);
                        ...
                        <div className='p-8 h-full space-y-6'>
                            ...
                            {walletStatus.isValidAddress === null || walletStatus.isValidAddress === false ? (
                                ...
                                <div className='mx-auto relative w-[80%]'>
                                    ...
                                    />
                                    <button type="button" className="hover:cursor-pointer" onClick={validate_Address}>
                            ...
                            {(walletStatus.inValidForm === true && walletStatus.loginError == "") && (<label className='text-red text-xs flex justify-center animate-pulse'>Missing Fields...</label>)}
